...
class EventViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
  ...
  func getItems() {
    ...
    Alamofire.request("https://chalie-vice-api.herokuapp.com/chalie_vice/letters.json?category=FROM+CHALIE")
  ...
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    if index >= 0 {
      selectedIndex = index
      performSegue(withIdentifier: "showMagazineFromEvent", sender: nil)
    }
  }
  
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    if segue.identifier == "showMagazineFromEvent" {
